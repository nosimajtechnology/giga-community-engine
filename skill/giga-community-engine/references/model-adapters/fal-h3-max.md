...
Control`, `Direct Explore`, or `Character Lock`. Apply GIGA canon, selected style
or game build, approved continuity, and the model-neutral animation brief first.
...
iterate freely, recommend DIRECT EXPLORE. If they want recognizable GIGA without
a fixed opening frame, recommend CHARACTER LOCK. Ask the compact chooser from
...
openings, repetitive angles, slideshow pacing, impossible contact, crowded
body mechanics, and an unresolved final beat.

For transformations, state `PRE-STATE -> physical impact or occlusion bridge ->
POST-STATE`. Never request continuous liquid morphing of GIGA's face, jaw,
hairline, beard, body, anatomy, or clothing.
...
contains camera, GIGA or subject action, environmental response, and the
...
IDENTITY INVARIANTS:
[protected GIGA traits and current wardrobe/state]
...
budget on motion, contact points, camera, continuity, and endpoint.
...
GIGA completely enough to stand alone:

```text
The protagonist is the recognizable Gigachad: a very tall, powerfully muscular
adult man with warm tan skin, a long angular face, heavy brow, deep-set narrow
eyes, strong cheek planes, a long straight-to-angular nose, and a broad squared
jaw. His dark hair is combed and slicked straight back with a controlled
hairline. He has a close shaped full dark beard connected to the moustache,
never clean-shaven or bushy. Preserve his thick neck, extremely broad shoulders,
narrow waist, long powerful limbs and legs, stoic composure, and exact silhouette
in every shot. He is not a generic handsome bodybuilder.
```
...
identity as interpretive. If the face, hairline, beard, or physique repeatedly
drifts, recommend CHARACTER LOCK or CLASSIC CONTROL rather than making the
prompt longer and more conflicted.
...
```text
Image 1 = ../../assets/style-adapters/late-z-battle-cel/giga-late-z-character-sheet-v1.png;
sole default reference and complete authority for GIGA identity, face, hair,
beard, anatomy, costume, proportions, palette, linework, cel shading, and
...
Do not also upload the canonical GIGA sheet or raw broadcast frames. Add
...
```text
Image 1 = ../../assets/giga-character-sheet.png; immutable GIGA identity,
face, hairline, beard, physique, proportions, and silhouette.
Image 2 = selected adapter-specific GIGA sheet when a future adapter's existing
